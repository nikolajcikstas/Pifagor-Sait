...
import { useFaq, usePrices, useSubjects } from "./hooks/usePublicData";
import {
  findSubjectIdByKey,
  mapApiFaq,
} from "./utils/tutors";
...
const CHEMISTRY_BANNER_DECOR: {
  ...
  { top: "-8%", left: "-6%", width: "420px", rotate: -28, opacity: 0.35 },
  { top: "12%", left: "18%", width: "360px", rotate: 42, opacity: 0.28 },
  { top: "-5%", right: "8%", width: "390px", rotate: 75, opacity: 0.3 },
  { top: "35%", right: "-4%", width: "480px", rotate: -55, opacity: 0.32 },
  { bottom: "18%", left: "4%", width: "440px", rotate: 18, opacity: 0.28 },
  { bottom: "8%", right: "22%", width: "320px", rotate: -90, opacity: 0.25 },
  { top: "48%", left: "42%", width: "280px", rotate: 135, opacity: 0.22 },
];
...
const SUBJECT_CONFIGS: Record<SubjectKey, SubjectConfig> = {
  ...
  physics: {
    ...
    tutors: [
      ...
      { name: "Джон Гиллман", title: "Я умнее, сильнее. Я лучше. Я лучше вас!" },
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {faq.question.endsWith('?') ? <>{faq.question.slice(0, -1)}<span className="banner-form-punct">?</span></> : faq.question}
        </span>
...
export default function SubjectPage({ subject, onBack, onNavigate }: SubjectPageProps) {
  ...
  const tutors = useMemo(() => {
    return TUTORS.filter(
      (t) => t.subjects.some(s => s.toLowerCase() === cfg.title.toLowerCase())
    );
  }, [cfg.title]);
